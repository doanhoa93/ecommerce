...
public class FakeData {
  ...
  public static void addProducts(Session session) {
    try {
      ...
      List<Category> categories = (List<Category>) session.createCriteria(Category.class)
        .add(
          Restrictions.in("id", new ArrayList<Integer>(Arrays.asList(1, 2, 3, 4, 5, 6, 7, 8, 9))))
        .list();
      for (int i = 1; i < 10; i++) {
  ...
  @SuppressWarnings({ "deprecation", "unchecked" })
  public static void addCartProducts(Session session) {
    try {
      Transaction t = null;
      t = session.beginTransaction();
      session.createQuery("delete from CartProduct").executeUpdate();
      t.commit();

      t = session.beginTransaction();
      List<Cart> carts = (List<Cart>) session.createCriteria(Cart.class)
        .add(
          Restrictions.in("id", new ArrayList<Integer>(Arrays.asList(1, 2, 3, 4, 5, 6, 7, 8, 9))))
        .list();
      ...
      for (int i = 1; i < 10; i++) {
        ...
        cartProduct.setId(i);
        cartProduct.setProduct(products.get(i - 1));
        cartProduct.setCart(carts.get(i - 1));
